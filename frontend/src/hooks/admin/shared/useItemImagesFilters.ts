import { UseFiltersOptions, useFilters } from '@/hooks/filters/useFilters';
import { ItemImagesFiltersDto } from '@lib/api/services/types/item-images.types';

const DEFAULT_FILTERS: ItemImagesFiltersDto = {
  product_id: '',
  service_id: '',
  skip: 0,
  take: 10,
};

/**
 * Хук для управления фильтрами изображений товаров и услуг с дебонсом
 * @param options - Дополнительные опции для хука фильтров
 */
export const useItemImagesFilters = <T extends ItemImagesFiltersDto>(
  options?: Partial<UseFiltersOptions<T>>
) => {
  return useFilters<T>({
    default_filters: options?.default_filters || (DEFAULT_FILTERS as T),
    debounce_settings: {
      fields: ['search'],
      delay: 1000
    },
    permanent_fields: options?.permanent_fields
  });
}; 