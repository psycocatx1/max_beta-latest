import { UseFiltersOptions, useFilters } from '@/hooks/filters/useFilters';
import { ProductFiltersDto } from '@lib/api/services/types/products.types';

const DEFAULT_FILTERS: ProductFiltersDto = {
  category_id: '',
  skip: 0,
  take: 10,
};

/**
 * Хук для управления фильтрами товаров с дебонсом
 * @param options - Дополнительные опции для хука фильтров
 */
export const useProductsFilters = <T extends ProductFiltersDto>(
  options?: Partial<UseFiltersOptions<T>>
) => {
  return useFilters<T>({
    default_filters: options?.default_filters || (DEFAULT_FILTERS as T),
    debounce_settings: {
      fields: ['search'],
      delay: 1000
    },
    permanent_fields: options?.permanent_fields
  });
}; 